...
top = plot(ω_array, trace_dist_array_mean,ylabel=L"\mathrm{TrDist}(\rho_{ss},\rho_{th})", xlabel=L"ω",legend=false)

mid = plot(ω_array, beta_array_mean, yerr = beta_array_std,c=:black,lw=2,ylabel=L"\beta_{eff}", xlabel=L"ω",label="sim")
...
top = plot(ω_array, temperature_array,legend=false,c=:black,lw=2,ylabel=L"T_{eff}", xlabel=L"ω")
plot!(ω_array,temp_nth.(n_th,ω_array),lw=2,ls=:dash)

bot = plot(ω_array, std(temperature_array*1000,dims=2)*1000,legend=false,c=:black,ms=2,ylabel=L"\sigma(T_{eff})"*" (mK)", xlabel=L"ω")
