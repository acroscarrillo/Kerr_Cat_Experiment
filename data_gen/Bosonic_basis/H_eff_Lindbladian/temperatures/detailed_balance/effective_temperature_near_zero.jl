...
K = 1
Δ = -0*K
ϵ_1_array = Vector(range(-0.05,0.05,length=99))
ϵ_2 = 7.7 # Max's value
...
top = plot(ϵ_1_array,trace_dist_array_mean,ylabel=L"\mathrm{TrDist}\left(\rho_\mathrm{ss},\rho_\mathrm{th}\right)",lw=1.5,c=:black, xlabel=L"ϵ_1/K",legend=false;plot_kwargs...)
# vline!([ϵ_1_reso.(ϵ_2,[1,2,3],1)...],c=:orange,lw=2,ls=:dash,alpha=0.8)
# vline!([ϵ_1_reso.(ϵ_2,[4],1)...],c=:purple,lw=2,ls=:dash,alpha=0.8)


mid = plot(ϵ_1_array, abs.(beta_array_mean),yerr = beta_array_std,legend=false,ylim=(-0.3,0.5),c=:black,ms=2,ylabel=L"\beta_\mathrm{avg}", xlabel=L"ϵ_1/K")
plot!(ϵ_1_array, abs.(beta_array_mean),c=:red)
