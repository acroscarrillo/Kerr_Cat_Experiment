...
function get_beta_array(ρ,H,N_thresh)
    ...
    for n=2:N_thresh
        ...
        if p_n>1e-6 && p_m>1e-6
            # display("p_n=$p_n, p_m=$p_m")
            # display("p_n_indx=$(p_indx[n]), p_m_indx=$(p_indx[m])")
            # display("p_n=$p_n, p_m=$p_m")
...
K = 1
Δ = -0*K
ϵ_1_array = Vector(range(0.01,15,length=180))
ϵ_2_array = Vector(range(0.01,12,length=179))
...
beta_array_mean = zeros( length(ϵ_1_array),length(ϵ_2_array) )
beta_array_std = zeros( length(ϵ_1_array),length(ϵ_2_array) )
trace_dist_array_mean = ones( length(ϵ_1_array),length(ϵ_2_array) )
...
for (j,ϵ_1) in enumerate(ϵ_1_array)
    for (k,ϵ_2) in enumerate(ϵ_2_array)
        ...
        beta_array_mean[j,k] = temp[1]
        beta_array_std[j,k] = temp[2]

        ρ_th = exp(-temp[1] * H_temp) / tr(exp(-temp[1] * H_temp))
        if !(any(x -> isnan(x) || isinf(x), ρ_th))
            trace_dist_array_mean[j,k] = tr_dist( ρ_ss_mat/norm(tr(ρ_ss_mat)), ρ_th)
        end
        update(pbar)
    end
end
# npzwrite("data_gen/Bosonic_basis/H_eff_Lindbladian/temperatures/beta_array_mean_N_$(N)$N_thresh.npz", beta_array_mean)

top = heatmap(ϵ_1_array,ϵ_2_array,log.(trace_dist_array_mean'),ylabel=L"ϵ_2/K", xlabel=L"ϵ_1/K",colorbartitle=L"\mathrm{TrDist}(\rho_{ss},\rho_{th})")

mid = heatmap(ϵ_1_array,ϵ_2_array,beta_array_mean',ylabel=L"ϵ_2/K", xlabel=L"ϵ_1/K",colorbartitle=L"\langle \beta_\mathrm{avg} \rangle",clim=(0,2))

bot = heatmap(ϵ_1_array,ϵ_2_array,log.(beta_array_std'),ylabel=L"ϵ_2/K", xlabel=L"ϵ_1/K",colorbartitle=L"\sigma( \beta_{eff} )")


l = @layout [top; mid; bot]

width_px=340.39020340390204
plot(top,mid,bot,layout =l,size = (width_px,width_px*1.6),xtickfontsize=8,ytickfontsize=8,dpi=600,widen=false,ylabelfontsize=8,xlabelfontsize=8,tickdirection=:out,fontfamilys = "Times New Roman",tickfontfamily = "Times New Roman",axislabelfontfamily="Times New Roman",grid=false,plot_title="N=$N, N_thresh=$N_thresh, only nn.",plot_titlefontsize=8)


savefig("data_gen/Bosonic_basis/H_eff_Lindbladian/temperatures/heatmap_N_$(N)_N_thresh_$(N_thresh).png")
savefig("data_gen/Bosonic_basis/H_eff_Lindbladian/temperatures/heatmap_N_$(N)_N_thresh_$(N_thresh).pdf")