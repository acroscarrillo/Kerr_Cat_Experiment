...
function evolve!(ϵ_2, X_op, t_array, ϕ_mat, ϕ_mat_inv, λ_n, ρ_0_vec, ρ_t_vec)
    # Precompute v0 = Φ⁻¹ * ρ0 once (saves one matvec per time)
    v0 = similar(ρ_0_vec)
...
# Prealocate memory
H_temp_super = H_super_op( -H_eff(N,Δ,K,1,ϵ_2) )
L_temp = H_temp_super + C_ops_super
λ_n, ϕ_mat = eigen(L_temp) 
ϕ_mat_inv = inv(ϕ_mat)

exp_tL = zeros(ComplexF64, N^2, N^2)
ρ_t_vec = zeros(ComplexF64, N^2)
ρ_t_mat = zeros(ComplexF64, N, N)
exp_λt = zeros(ComplexF64, N^2)
temp_mat = zeros(ComplexF64, N^2, N^2)
...
plot(ϵ_1_array, fit_array[:,1], ylab = L"\tau K", xlab= L"\epsilon_1/K",legend=false, grid=false)

plot!(ϵ_1_array, T1_exp_array_unitless)